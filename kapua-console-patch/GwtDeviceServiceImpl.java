...
public class GwtDeviceServiceImpl extends KapuaRemoteServiceServlet implements GwtDeviceService {
    ...
    // ── Real TCP IP lookup via debug-proxy ────────────────────────────────────
    ...
    private static Map<String, String> fetchRealIpMap() {
        ...
        if (now - _realIpCacheTime < 10_000L) {
            return _realIpCache;
        }
        try {
            java.net.URL url = new java.net.URL("http://debug-proxy:7080/internal/netstat-ips");
    ...
    public PagingLoadResult<GwtDevice> query(PagingLoadConfig loadConfig, GwtDeviceQuery gwtDeviceQuery)
        ...
        try {
            ...
            Map<String, String> realIpMap = fetchRealIpMap();
            for (Device d : devices.getItems()) {
                ...
                if (deviceConnection != null) {
                    ...
                    // Override with real TCP source IP from debug-proxy if available
                    String realIp = realIpMap.get(d.getClientId());
                    if (realIp == null && DeviceConnectionStatus.CONNECTED.equals(deviceConnection.getStatus())) {
                        // Single-device fallback: 1 TCP connection, no BIRTH event yet
                        realIp = realIpMap.get("__single__");
                    }
